/**************************************************************
* Description: We're using threads to help parse through a
* a text file and store the word frequency of words with
* six or more characters. We then display the words with the
* top 10 frequency.
**************************************************************/
...
word_struct* words_container = NULL; 	/* to hold words */
int container_count = 0;		/* number of 6+ char words currently in words_container */
int container_size = 0;			/* the size of words_container */
...
void add_to_container(char* word)
{
	...
	if(container_count >= container_size)
	{
		...
		word_struct* realloc_words_container = realloc(words_container, sizeof(word_struct) * container_size);
...
int top_ten_sort(word_struct* top)
{
	...
	if(i < container_count)
		...
		do
		{
                	if(words_container[i].word_count > top[k].word_count)
                	{
...
int main (int argc, char *argv[])
	...
	while(i < TOP_TEN)
	{
		top[i++].word_count = 0; /* initialize word counts */
	}
	/* sort top 10 */
	top_ten_sort(top);

    // Process TOP 10 and display
	printf("\nWord Frequency Count on WarAndPeace.txt with %d threads\n", proc_count);
	printf("Printing top 10 words with %d characters or more.\n", SIX_CHARS);
	i = 0;
	while(i < TOP_TEN)
	{
